...
/**
 * Because  player states can be send over network it sometimes comes to collision between players and they could move through each other.
 * To fix this if a collision is detected we move each player backwards until no collision exists anymore.
 *
 */
public class FixPlayerPosition implements PlayerJoinListener {
	...
	private void fixCollision(Player player1, Player player2) {
		LOGGER.info("Collision between players %s and %s. Fixing this by moving them backwards.", player1.getName(), player2.getName());
		int maxTries = 10; //limit tries to avoid endless loop which might happen otherwise if players move at the same speed
